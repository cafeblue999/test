...
# ビットパック時の横幅（8ビットごとにパック）
PACKED_WIDTH = ceil(BOARD_SIZE / 8)
...
class AlphaZeroSGFDatasetPreloaded(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # board_tensor の生成：ビットパック配列 (uint8) / numpy float配列 / Tensor を判別
        ...
        #        .to(torch.bfloat16)
        #    )
        elif isinstance(inp, np.ndarray) and inp.dtype == np.uint8:
            # ビットパックされた入力をアンパック
            packed = inp.reshape(NUM_CHANNELS, BOARD_SIZE, PACKED_WIDTH)  # C×H×PACKED_WIDTH
            bits   = np.unpackbits(packed, axis=-1)[..., :BOARD_SIZE]     # C×H×W
            # float32に戻してからTensor化
            arr    = bits.astype(np.float32)
            board_tensor = (
...
def process_sgf_to_samples_from_text(sgf_src, board_size, history_length, augment_all):
    ...
    for node in sgf_data["nodes"]:
        ...
        for t in transforms:
            ...
            # 1) dihedral変換して浮動小数点テンソル生成
            inp = apply_dihedral_transform(input_tensor, t)  # shape=(C,H,W), float32
            pol = transform_policy(target_policy, t, sz)
            # 2) 真偽値化→uint8化→ビットパック（axis=-1方向, 8列ごとにパック）
            bitplanes = (inp > 0).astype(np.uint8)           # 0/1 uint8
            packed   = np.packbits(bitplanes, axis=-1)       # shape=(C,H,PACKED_WIDTH)
            samples.append((
                packed.flatten(),                           # ビットパックした配列を1次元化
                pol,                                        # policyラベル
                np.array([target_value], dtype=np.float32),# valueラベル
            ))
